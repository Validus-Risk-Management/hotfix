...
pub fn prepare_message_for_resend(msg: &mut Message) -> Result<(), &'static str> {
    ...
    if header.get_raw(fix44::SENDING_TIME).is_none() {
        return Err("Missing SendingTime (52)");
    }

    let already_poss_dup = header.get::<bool>(fix44::POSS_DUP_FLAG).unwrap_or(false);
    let has_orig_sending_time = header.get_raw(fix44::ORIG_SENDING_TIME).is_some();

    if already_poss_dup && has_orig_sending_time {
        // Subsequent resend: refresh SendingTime only
        return if header.pop(fix44::SENDING_TIME).is_some() {
    ...
    // First resend path
    if let Some(original_sending_time_field) = header.pop(fix44::SENDING_TIME) {
        let original_ts = Timestamp::parse(&original_sending_time_field.data)
            .ok_or("Invalid original SendingTime format")?;
        header.set(fix44::ORIG_SENDING_TIME, original_ts);
        header.set(fix44::SENDING_TIME, Timestamp::utc_now());
